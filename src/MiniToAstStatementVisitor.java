...
public class MiniToAstStatementVisitor extends MiniBaseVisitor<Statement> {
   ...
   public Statement visitAssignment(MiniParser.AssignmentContext ctx) {
      ...
      return new AssignmentStatement(ctx.getStart().getLine(), visitLvalue(ctx.lvalue()), expression);
   }

   @Override
   public Statement visitPrint(MiniParser.PrintContext ctx) {
      return new PrintStatement(ctx.getStart().getLine(), expressionVisitor.visit(ctx.expression()));
   }

   @Override
   public Statement visitPrintLn(MiniParser.PrintLnContext ctx) {
      return new PrintLnStatement(ctx.getStart().getLine(), expressionVisitor.visit(ctx.expression()));
   }
   ...
   public Statement visitReturn(MiniParser.ReturnContext ctx) {
      if (ctx.expression() != null) {
         return new ReturnStatement(ctx.getStart().getLine(), expressionVisitor.visit(ctx.expression()));
      } else {
   ...
   private Lvalue visitLvalue(MiniParser.LvalueContext ctx) {
      if (ctx instanceof MiniParser.LvalueIdContext) {
         ...
         return new LvalueDot(lctx.getStart().getLine(), visitLvalueNested(lctx.lvalue()), lctx.ID().getText());
      }
   }
   ...
   private Expression visitLvalueNested(MiniParser.LvalueContext ctx) {
      if (ctx instanceof MiniParser.LvalueIdContext) {
         ...
         MiniParser.LvalueDotContext lctx = (MiniParser.LvalueDotContext)ctx;
         return new DotExpression(lctx.getStart().getLine(), visitLvalueNested(lctx.lvalue()), lctx.ID().getText());
      }
   }
